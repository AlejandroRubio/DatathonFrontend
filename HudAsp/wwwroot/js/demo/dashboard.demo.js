...
var handleRenderChart = function () {
    ...
    var salarioChartOptions = {
        ...
        series: [{
            name: 'Media',
            data: [
                21883.42, 22511.47, 22790.20, 22899.35, 22726.44, 22697.86, 22858.17, 23106.30, 23156.34, 23646.50, 24009.12, 24395.98, 25165.51, 25896.82
            ]
        }, {
            name: 'Mediana',
            data: [
                18244.40, 18803.28, 19017.09, 19303.51, 19040.98, 19029.66, 19263.78, 19466.49, 19432.62, 19830.12, 20078.44, 20351.02, 20920.12, 21638.69
            ]
            },
            {
                name: 'Cuartil Superior',
                data: [
                    26874.02, 28024.86, 28255.45, 28658.66, 28395.50, 28563.69, 28782.70, 29163.66, 29191.56, 29628.64, 30057.33, 30558.45, 31550.01, 32385.45

                ]
            }
        ],
        xaxis: {
            categories: ['2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016', '2018', '2019', '2020', '2021'],
            labels: {
                show: false
            }
        },
    ...
    var apexSalarioChart = new ApexCharts(
        document.querySelector('#chart-salario'),
        salarioChartOptions
    );
    apexSalarioChart.render();
};
...
$(document).ready(function () {
    ...
    document.addEventListener('theme-reload', function () {
        $('[data-render="apexchart"], #chart-server,  #chart-salario, #world-map').empty();
        handleRenderChart();
        handleRenderMap();
    });
});